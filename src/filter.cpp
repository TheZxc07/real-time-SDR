...
void convolveFIR(const int over, std::vector<float> &y, const std::vector<float> &x, const std::vector<float> &h, std::vector<float> &initial_state, const int upsample, const int decimation_factor)
{
	y.clear(); y.resize(x.size()/decimation_factor);
	int phase;
	for (int n = 0; n < x.size(); n+=decimation_factor) {
		for (int k = 0; k < h.size(); k+=upsample){
			phase = n % upsample;
			if (n-k < 0) {
				y[n/decimation_factor] += (short int)((16384*h[k]*initial_state[n-k+initial_state.size()]));
			}else{
